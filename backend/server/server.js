    ...
    db.query(checkUserQuery, [identity_number], async (err, results) => {
      ...
      db.query(
        ...
        (err, customerResult) => {
          ...
          db.query(
            insertUserQuery,
            [customerResult.insertId, identity_number, passwordHash],
            (err) => {
              if (err) {
                return res.status(500).json({ error: "Database error" });
              }
...
app.post("/verify-token", authenticateToken, async (req, res) => {
  try {
    ...
    const [results] = await db
      .promise()
      .query(fullNameQuery, [req.user.identity]); // מחכים לתוצאה של השאילתא
    ...
    if (
      !(
        await db
          .promise()
          .query(
            "SELECT EXISTS(SELECT 1 FROM Products WHERE product_id = ?) AS `exists`",
            [product_id]
          )
      )[0][0].exists
    ) {
      return res
        ...
        .json({
          valid: false,
          error: "The product is no longer in stock.",
          code: "OUT_OF_STOCK",
        });
    }
...
app.delete("/api/clearCart", authenticateToken, async (req, res) => {
  ...
  try {
    await db
      .promise()
      .query("DELETE FROM cartitems WHERE customer_id = ?", [userId]);
